...
Le VC renseigne au minimum un secteur (`climate tech`, `fintech`, `healthtech`, etc.). Il peut ensuite préciser sa thèse d'investissement : stade, zone géographique, ticket, modèle économique, niveau de risque ou mots-clés. L'outil recherche les opportunités compatibles dans un catalogue, les analyse, les classe et présente un **top 5 argumenté**.
...
- un top 5 accompagné de justifications, risques et informations manquantes ;
...
2. Il précise éventuellement son stade, sa géographie, son ticket et ses priorités.
3. Le système filtre les idées non pertinentes.
4. Il analyse et score les candidates restantes avec une grille commune.
5. Il classe les idées et retourne les cinq meilleures.
...
Filtrer le catalogue sur les critères explicites : domaine, géographie, stade, ticket ou business model. Un critère non renseigné ne doit pas éliminer une idée.
...
9. prompt V2 produit et génération du top 5 ;
...
- cinq cartes de recommandation triées ;
...
9. Calculer le score et générer un top 5 expliqué.
...
- [ ] le moteur retourne un top 5 structuré et expliqué ;
...
Avant de générer les données ou coder l'interface, organiser une réunion courte et figer ces cinq décisions :

1. les six domaines du dataset ;
2. les champs obligatoires d'une idée ;
3. les filtres accessibles au VC dans le MVP ;
4. la grille de score et ses pondérations ;
5. le modèle LLM utilisé pour la première version.

Une fois ces décisions validées, commencer par `feature/data-schema`, puis produire un échantillon de deux idées par domaine. Valider cet échantillon avant de générer les 60 idées.
